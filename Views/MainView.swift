...
private let viewLogger = Logger(subsystem: "com.wqm", category: "MainView")
    ...
    var body: some View {
        ZStack {
            ...
            if !networkVM.isConnectedToWiFi {
                let _ = viewLogger.error("🚫 Showing BlockedView: noWiFi (isConnectedToWiFi=\(networkVM.isConnectedToWiFi))")
                BlockedView(type: .noWiFi)
            } else if !networkVM.hasLocalNetworkPermission {
                let _ = viewLogger.error("🚫 Showing BlockedView: noPermission (hasLocalNetworkPermission=\(networkVM.hasLocalNetworkPermission))")
                BlockedView(type: .noPermission)
            } else {
                let _ = viewLogger.info("✅ Showing mainContent")
                mainContent
            }
